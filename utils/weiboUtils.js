    ...
    commonSend: function(data, callback, isAuto) {
        module.exports.checkOauth(function(token) {
            ...
            if (data.title && data.description && data.mkt.indexOf('zh-cn') > -1) {
                var status = ('#必应壁纸# ' + full + ' / #' + data.title + '# ' + data.description).slice(0, 138) + `...` + ROOT + `/photo/${data.qiniu_url}?from=weibo`;
                post = {
                    access_token: token,
                    status: status,
                    //url: data.url,
                ...
                var status = '#Bing Picture# ' + full + ' / ' + data.copyright + ` from ${data.mkt}` + ROOT + `/photo/${data.qiniu_url}?from=weibo`;
                post = {
                    access_token: token,
                    status: status,
                    //url: data.url,
